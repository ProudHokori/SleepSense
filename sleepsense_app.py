...
def build_prep(df_session):
    """Dynamically builds the preprocessing pipeline based on actual feature skew."""
    SKEW_THRESHOLD = 0.75
    
    # Measure skew dynamically just like the notebook
    feature_skew = {
...
def load_and_train():
    ...
    # Apply the exact same GroupShuffleSplit used in the notebook experiment
    gss = GroupShuffleSplit(n_splits=1, test_size=0.35, random_state=42)
    train_idx, test_idx = next(gss.split(X, y, groups=groups))
    ...
    # Fit preprocessing EXCLUSIVELY on the training split
    prep = build_prep(df_session)
    prep.fit(X_train, y_train)
    feat_names = list(prep.get_feature_names_out())
    X_t = prep.transform(X_train)
    ...
    # Build SHAP explainers using the training data
    explainers = {}
